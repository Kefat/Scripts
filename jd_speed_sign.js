...
function taskUrl(searchSortBy, message = {}) {
  ...
  let challenge = `lite-android&${JSON['stringify'](
      message
  )}&android&3.1.0&${searchSortBy}&${i}&846c4c32dae910ef`;
  let salt = '12aea658f76e453faf803d15c40a72e0';
  $.CryptoJS = require('crypto-js');
  let SP = $.CryptoJS.HmacSHA256(challenge, salt).toString();
  return {
    url: `${JD_API_HOST}api?functionId=${searchSortBy}&body=${escape(
        JSON['stringify'](message)
    )}&appid=lite-android&client=android&uuid=846c4c32dae910ef&clientVersion=3.1.0&t=${i}&sign=${SP}`,
    headers: {
      Host: 'api.m.jd.com',
      accept: '*/*',
      kernelplatform: 'RN',
