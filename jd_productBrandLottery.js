/*
特务Z，只抽奖，不做任务
50,55 23 * * * jd_productBrandLottery.js
*/
const $ = new Env('特务Z抽奖');
...
!(async () => {
  ...
  for (let key of myMap.keys()) {
    if (myMap.get(key) > 0) message += `账号 ${key}\n获得京豆：${myMap.get(key)}\n\n`;
  }
  if (message) {
    $.msg($.name, '', message);
...
async function main(taskCookie, index) {
  const userName = decodeURIComponent(taskCookie.match(/pt_pin=([^; ]+)(?=;?)/) && taskCookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
  myMap.set(userName, 0)
  $.activityInfo = {};
  if (!$.activityId) {
    const data = await takeRequest('superBrandSecondFloorMainPage', taskCookie, index);
    if (data.code === '0' && data.data && data.data.result) {
      $.activityInfo = data.data.result;
      console.log(`账号 ${index + 1} ${userName}获取活动详情成功`);
      $.activityId = $.activityInfo.activityBaseInfo.activityId;
    }
    if (JSON.stringify($.activityInfo) === '{}') {
      console.log(`账号 ${index + 1} ${userName}获取活动详情失败`);
      return;
    }
  }
  for (let i = 0; i < 10; i++) {
    console.log(`账号 ${index + 1} ${userName} 进行第${i + 1}次抽奖`);
    const data = await takeRequest('superBrandTaskLottery', taskCookie, index);//抽奖
    if (data.code === '0' && data.data.bizCode !== 'TK000') {
      ...
      if (reward && reward.beanNum) {
        console.log(`账号 ${index + 1} ${userName} 恭喜你 获得 ${reward.beanNum}京🐶\n`)
        myMap.set(userName, parseInt(myMap.get(userName)) + parseInt(reward.beanNum))
        // message += `账号 ${index + 1} ${userName}\n获得京豆：${parseInt(reward.beanNum)}\n\n`;
...
async function takeRequest(type, taskCookie, index) {
  let url = ``;
  let myRequest = ``;
  switch (type) {
  ...
  myRequest = getRequest(url, taskCookie);
  return new Promise(async resolve => {
    $.post(myRequest, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`京东账号${index + 1} API请求失败，请检查网路重试\n`)
        } else {
          data = $.toObj(data);
        }
      } catch (e) {
...
function getRequest(url, taskCookie) {
  const headers = {
