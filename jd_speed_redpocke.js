...
async function jsRedPacket() {
  try {
    // await invite();
    await sign();//极速版签到提现
    $.spring_reward_query = false;
    await reward_query();
    if (!$.spring_reward_query) {
      for (let i = 0; i < 3; ++i) {
        await redPacket();//开红包
        await $.wait(5000)
      }
...
async function sign() {
  return new Promise(async resolve => {
    ...
    const h5stBody = await getH5stBody('apSignIn_day', $.toStr(body));
    const options = {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              ...
              if (data && data.code === 1006) {
                console.log(`天天领红包失败，账号已黑`)
                $.spring_reward_query = true;
              }
            }
          }
        }
...
async function getH5stBody(functionId, bodyInfo) {
  ...
  let body = `functionId=${functionId}&body=${bodyInfo}&t=${signtime}&client=wh5&clientVersion=1.0.0&appid=activities_platform`
  const bodySign = $.CryptoJS.SHA256(bodyInfo).toString($.CryptoJS.enc.Hex);
  let url = `https://api.m.jd.com?functionId=${functionId}&body=${bodySign}&t=${signtime}&client=wh5&clientVersion=1.0.0&appid=activities_platform`;
  ...
  body += `&h5st=${encodeURIComponent(h5st)}`
  return body;
}
