/*
40 23 * * * jd_bean_detail.js
 */
try {
  var { Env } = require('./Env')
} catch (e) {}
const $ = new Env('京豆收益明细');
...
!(async () => {
  ...
  if (allMessage) {
    $.msg($.name, '', allMessage)
    if ($.isNode()) await notify.sendNotify(`${$.name}`, `${allMessage}`, { url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean` })
  }
})()
    .catch((e) => {
      $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
...
async function showMsg() {
  if ($.errorMsg) return
  allMessage += `${$.index === 1 ? '' : '\n'}【账号${$.index}】${$.UserName}\n`;
  allMessage += `【今日收入京豆🐶】${$.todayIncomeBean}个\n`
  let temp = [];
  for (let key of myMap.keys()) {
    temp.push({
      'amount': myMap.get(key),
      'msg': key.includes('参加') ? key.slice(3, key.indexOf(']')) : key
    })
  }
  temp.sort(sortByAmount);
  for (const item of temp) {
    allMessage += `【${item.amount}京豆🐶】${item.msg}\n`
  }
  myMap = new Map()
}
async function bean() {
