...
const invokeKey = $.isNode() ? require('./utils/config').invokeKey : 'q8DNJdpcfRQ69gIx';
    ...
    if (deskGoodsRes.data && deskGoodsRes.data.deskGoods) {
      ...
      for (let item of deskGoods) {
        if (!item['status'] && item['sku']) {
          console.log(`\n浏览限时货柜：${item['skuName']}`)
          await iconClick('follow_good_desk', item['sku'])
          await $.wait(5 * 1000)
...
function iconClick(functionId, id) {
  return new Promise(resolve => {
    $.get({
      url: `https://jdjoy.jd.com/common/pet/icon/click?code=1624363341529274068136&iconCode=${functionId}&linkAddr=${id}&reqSource=h5&invokeKey=${invokeKey}`,
      headers: {
        'Accept': '*/*',
        'Connection': 'keep-alive',
        'Content-Type': 'application/json',
