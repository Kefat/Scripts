  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        ...
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} 账号已过期 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录`);
        }
        continue
      }
      await bean();
      ...
      if (detailList && detailList.length > 0) {
        for (let item of detailList) {
          ...
          if (new Date(date).getTime() >= tm1 && (!item['eventMassage'].includes("退还") && !item['eventMassage'].includes('扣赠'))) {
            todayArr.push(item);
          } else if (new Date(date).getTime() < tm1) {
            //昨日的
...
function jingBeanDetail() {
  return new Promise(async resolve => {
    setTimeout(async () => {
      const s = await getSign('jingBeanDetail', {"pageSize": "20", "page": "1"});
      const options = {
          ...
          if (err) {
            ...
            if (data) {
              ...
              if (data?.others?.jingBeanExpiringInfo?.detailList) {
                console.log(`账号${$.index} ${$.UserName} ${data?.others?.jingBeanExpire?.title}\n`)
                const { detailList = [] } = data?.others?.jingBeanExpiringInfo;
                detailList.map(item => {
                  console.log(`账号${$.index} ${$.UserName} ${item['eventMassage']} ${item['amount']}个`);
                  $.expirejingdou += parseInt(item['amount'])
                })
              } else {
                console.log(`账号${$.index} ${$.UserName} ${data?.others?.jingBeanExpire?.title ? data?.others?.jingBeanExpire?.title : JSON.stringify(data)}`)
              }
            } else {
...
function redPacket() {
  return new Promise(async resolve => {
    const options = {
      "url": `https://api.jingxi.com/api?functionId=myassets.queryHongBao&appid=jx_h5&t=1683612501641&channel=jxh5&cv=1.2.5&clientVersion=1.2.5&client=jxh5&uuid=155988333990437&cthr=1&loginType=2&body=%7B%22listtype%22%3A1%2C%22orgFlag%22%3A%22JD_PinGou_New%22%2C%22page%22%3A1%2C%22cashRedType%22%3A1%2C%22redBalanceFlag%22%3A1%2C%22platform%22%3A3%2C%22sceneval%22%3A2%2C%22buid%22%3A325%2C%22appCode%22%3A%22msc588d6d5%22%2C%22time%22%3A1683612501641%2C%22signStr%22%3A%22911330fce74540dbd5f9ccbf75c6bd0a%22%7D`,
      "headers": {
        'Host': 'api.jingxi.com',
        'Accept': '*/*',
        'Connection': 'keep-alive',
      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            $.jxRed = 0, $.jsRed = 0, $.jdRed = 0, $.jdMiniRed = 0, $.jdOtherRed = 0;
            $.jxRedExpire = 0, $.jsRedExpire = 0, $.jdRedExpire = 0, $.jdMiniRedExpire = 0, $.jdOtherExpire = 0;
            let t = new Date()
            t.setDate(t.getDate() + 1)
            t.setHours(0, 0, 0, 0)
            t = parseInt((t - 1) / 1000)
            for (let vo of data.useRedInfo.redList || []) {
              if (vo.orgLimitStr && vo.orgLimitStr.includes("京喜渠道")) {
                $.jxRed += parseFloat(vo.balance)
                if (vo['endTime'] === t) {
                  $.jxRedExpire += parseFloat(vo.balance)
                }
              } else if (vo.orgLimitStr && vo.orgLimitStr.includes("京喜特价")) {
                $.jsRed += parseFloat(vo.balance)
                if (vo['endTime'] === t) {
                  $.jsRedExpire += parseFloat(vo.balance)
                }
              } else if (vo.orgLimitStr && vo.orgLimitStr.includes("京东购物小程序")) {
                $.jdMiniRed += parseFloat(vo.balance)
                if (vo['endTime'] === t) {
                  $.jdMiniRedExpire += parseFloat(vo.balance)
                }
...
function getSign(n, o) {
  return new Promise(async resolve => {
    const qs = require("querystring");
    let functionId = n, sign = '', body = '';
    const client = 'android';
    const clientVersion = '11.0.2';
    try {
      // const { getSign } = require('./utils/jd_sign');
      const getSign = require('./utils/jdSign3');
      sign = await getSign(functionId, $.toStr(o), client, clientVersion)
    } catch (e) {
      console.error(e);
    } finally {
      body = `body=${encodeURIComponent($.toStr(o))}&clientVersion=${clientVersion}&client=${client}&${qs.stringify(sign)}`
      resolve({body})
    }
  })
}
