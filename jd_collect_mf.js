...
20 4,21 * * * jd_collect_mf.js, tag=集魔方, enabled=true
 */
const $ = new Env('集魔方');
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        try {
            for (let index = 0; index < $.shareCodes.length; index ++) {
                ...
                console.log(`\n京东账号 ${$.index} ${$.UserName} 开始助力好友 ${$.userInviteInfo['user'] || ''}，itemId 为：【${$.userInviteInfo['itemId']}】`);
                const data = await doInteractiveAssignment($.projectId, $.userInviteInfo['encryptAssignmentId'], $.userInviteInfo['itemId'])
                if (data) {
                    if (data['code'] === '0' && data['subCode'] === '0') {
                        console.log('doInteractiveAssignment成功：' + $.toStr(data));
    ...
    try {
        ...
        for (const vo of $.taskList) {
            ...
            if (vo.ext.extraType === 'brandMemberList') {
                console.log(`任务：${vo.assignmentName}，进度：${vo.completionCnt}/${vo.assignmentTimesLimit}，跳过`);
                continue
            }
            if (vo.completionCnt < vo.assignmentTimesLimit) {
                console.log(`任务：${vo.assignmentName}，进度：${vo.completionCnt}/${vo.assignmentTimesLimit}，去完成`);
                if (vo.ext.extraType === 'assistTaskDetail') {
                    //好友助力码
                    if (vo.ext.assistTaskDetail.itemId && vo.encryptAssignmentId) {
                        $.shareCodes.push({
                            itemId: vo.ext.assistTaskDetail.itemId,
                            encryptAssignmentId: vo.encryptAssignmentId,
                            user: $.UserName,
                            max: false
                        })
                    }
                    console.log(`好友助力码：${vo.ext.assistTaskDetail.itemId}\n`)
                }
                ...
                for (let vi of (vo.ext.productsInfo || [])) {
                    if (vi.status === 1 && vo.completionCnt < vo.assignmentTimesLimit) {
        ...
        $.post(taskPostUrl("queryInteractiveInfo", body), async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    data = $.toObj(data);
                    if (data && data['subCode'] === '0' && data['code'] === '0') {
                        if (data.assignmentList && data.assignmentList.length) {
                            ...
                            if (reward) {
                                ...
                                if (item2) {
                                    for (const vo of item2) {
                                        const { rewards = [], exchangeRate } = vo;
                                        console.log(`\n消耗${exchangeRate}魔方可随机得：`)
...
function getInteractionHomeInfo() {
    ...
    return new Promise(resolve => {
        $.post(taskPostUrl("getInteractionHomeInfo", body), (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${err}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function taskPostUrl(function_id, body) {
    return {
        ...
        body: `functionId=${function_id}&body=${escape(JSON.stringify(body))}&appid=content_ecology&client=wh5&clientVersion=1.0.0`,
        headers: {
            "Host": "api.m.jd.com",
